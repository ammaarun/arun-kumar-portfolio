...
import { Sun, Moon, Menu, X, Command, FileText, Code2, Terminal, User, Briefcase, Mail } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-1 bg-slate-100/70 dark:bg-slate-900/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-slate-200/80 dark:border-slate-800/80">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="px-3.5 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-white dark:hover:bg-slate-800 rounded-full transition-all"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <button
            ...
            className="md:hidden p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-800 transition-colors"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white/95 dark:bg-[#0a0d14]/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 px-4 pt-3 pb-6 space-y-3 animate-fadeIn">
          {navLinks.map((link) => {
            ...
            return (
              <a
                ...
                className="flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/70 transition-colors"
              >
          ...
          <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
            <button
              ...
              className="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400 px-3 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg w-full justify-center"
            >
